...
const CarTransactionData = ({Cartns}) => {
    ...
    return (
        <>
         <Card className='mb-3 '  style={{backgroundColor:"#f2f2f2"}}>
            ...
            {(Cartns?.result?.length > 1) ? Cartns.result.map((car, index) => (
                <Card key={index} className="my-3">
                    <div className="bustnsvalue">
                        <Row className='px-3 py-2'>
                            <Col xs={2}>
                            <h6 className="fw-bold">{car.firstname}</h6>
                            </Col>
                            <Col xs={4}>
                            <span className='datainfo'>{car.txnid}</span><br/>
                            <span>({car.addedon})</span>
                            </Col>
                            <Col xs={3} className='text-center'>
                                <h6 className='fw-bold'>{car.mode}</h6>
                               <span className='datainfo'>{car.field8}</span>
                            </Col>
                            <Col>
                                <span className='h5 fw-bold text-danger'>{numberFormat(car.net_amount_debit, "INR")}</span>
                            </Col>
                            <Col xs={2}>
...
export default CarTransactionData;