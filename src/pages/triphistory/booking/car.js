import React, { useEffect, useState } from "react";
import '../triphistory.scss'
import { Tabs, Tab, Card, Row, Col, Button, Modal } from 'react-bootstrap';
import travels from '../../../asset/images/login/travels.png';
import moment from "moment";
import { carbookinglist } from "../../../store/services/car";
const TripMsg = () => {
    return (
        <>
            <Card>
                <Row>
                    <Col xs={4} className='p-4'>
                        <img src={travels} alt="travels" width={250} />
                    </Col>
                    <Col xs={5} className='mt-5'>
                        <h6>Looks like you have never booked with Hojoy
                            <span className='h5 fw-bold'>When you book your trips your transaction will be shown here.</span></h6>
                        {/* <Button className='mt-3'>Start Booking Now</Button> */}
                    </Col>
                </Row>
            </Card>
        </>
    )
}


const CarBookingData = ({ Carbook }) => {
    const [carBookingListData, setCarbookingdata] = useState();

    useEffect(() => {
        loadCarbookinglist();
    }, []);

    const loadCarbookinglist = async () => {
        let carbookingdata = await carbookinglist();
        setCarbookingdata(carbookingdata)
    };

    const numberFormat = (value, cur) =>
        new Intl.NumberFormat('en-IN', {
            style: 'currency',
            currency: cur,
            maximumFractionDigits: 0
        }).format(value);

    return (
        <>
            <Card className='mb-3 ' style={{ backgroundColor: "#f2f2f2" }}>
                <Card.Body>
                    <Row className='fw-bold ms-2'>
                        <Col xs={2}>Booking ID</Col>
                        <Col xs={2}>Operator ID</Col>
                        <Col xs={1}></Col>
                        <Col xs={2}>Transaction Id</Col>
                        <Col >created Date</Col>

                        <Col>Status</Col>

                    </Row>
                </Card.Body>
            </Card>
            {(carBookingListData?.result?.length > 0) ? carBookingListData.result.map((car, index) => (
                <div key={index} className="my-3">
                    <div className="bustnsvalue">
                        <Row className='px-3 py-2'>
                            <Col xs={2}>
                                <h6 className="fw-bold">{car.BookingID}</h6>
                            </Col>
                            <Col xs={2}>
                                <span className='datainfo'>{car.OperatorID}</span><br />
                            </Col>
                            <Col xs={3} className='text-center ms-5'>
                                <h6 className='fw-bold'>{car.transactionId}</h6>
                            </Col>
                            <Col>
                                <span className=''>{moment(car.createdAt).format("DD/MM/YYYY HH:MM")}</span>
                            </Col>
                            <Col xs={2}>
                                <span className='float-end h6 text-success'>{car.Status}</span>
                            </Col>
                            <Col></Col>
                        </Row>
                    </div>
                    {(car.BookingStatus === "Upcoming") ? (
                        <div className="text-end me-5">
                            <button className="btn btn-danger btn-sm" >Cancel Booking</button>
                        </div>
                    ) : null}
                </div>
            )) : <TripMsg />}

        </>
    )

}
export default CarBookingData;