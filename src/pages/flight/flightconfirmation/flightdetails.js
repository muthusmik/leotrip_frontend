                                                    ...
                                                    >
                                                        ...
                                                        <Tab eventKey="Baggage" title="Baggage">
                                                            <div>
                                                                ...
                                                                <div>
                                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="2rem" height="2rem" class="Flight__FlightIcon-sc-1w228os-0 eiNikk" loading="lazy"><path d="M31.95 16.262c-.012 0-.012 0 0 0-.337.762-2.238 1.3-2.238 1.3l-8.137-.012-8.863 12.925-2.088-.15 4.063-10.875-.225-1.95-9.588-.025L.499 22.05l-.5.063 1.438-5.663-.475-.375-.113-.088.125-.1.475-.363L.024 9.849l.5.063 4.338 4.6h9.6l.225-1.95-4.025-10.887 2.088-.15 8.825 12.963 8.137.012s2.713.762 2.238 1.763z"></path></svg>
                                                                    <span className='h6 mx-1'>{flightinfo.data[0]?.Results?.Segments[idx][0]?.Origin?.CityName}({flightinfo?.data[0]?.Results?.Segments[idx][0]?.Origin.CityCode}) - {HandleDestination(flightinfo.data[0]?.Results?.Segments[idx])}</span>
                                                                    {/* <span className='h6 small'>({flightinfo.data[0].Results.Segments[idx][0].Airline.AirlineName})</span> */}
                                                                </div>
                                                                <div className='mt-3'>
                                                                    {flightinfo.data[0]?.Results?.Segments[idx]?.length > 0 && flightinfo.data[0]?.Results?.Segments[idx]?.map((data) => (
                                                                        ...
                                                                        <div className='CancellationCharges'>
                                                                            {(flightoneway.data[0]?.result?.Results) ? flightoneway.data[0].result.Results.map((cancel, index) => (
                                                                                <div className='table-style ms-4' key={index}>
                                                                                    ...
                                                                                    <div>
                                                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="2rem" height="2rem" class="Flight__FlightIcon-sc-1w228os-0 eiNikk" loading="lazy"><path d="M31.95 16.262c-.012 0-.012 0 0 0-.337.762-2.238 1.3-2.238 1.3l-8.137-.012-8.863 12.925-2.088-.15 4.063-10.875-.225-1.95-9.588-.025L.499 22.05l-.5.063 1.438-5.663-.475-.375-.113-.088.125-.1.475-.363L.024 9.849l.5.063 4.338 4.6h9.6l.225-1.95-4.025-10.887 2.088-.15 8.825 12.963 8.137.012s2.713.762 2.238 1.763z"></path></svg>
                                                                                        <span className='h6 text-success  mx-1'>{segment[index]?.Origin?.CityName}({segment[index]?.Origin?.CityCode}) - {HandleDestination(segment)}</span>
                                                        ...
                                                        <div className="col-3 small my-auto">
                                                            <div>
                                                                <div className="justifyBetween alignItemsCenter d-flex">
                                                                    ...
                                                                    <p className="text-danger mt-1">{HandleBtw(flightinfo.data[0]?.Results?.Segments[idx][index])}</p>.....
                                                                ...
                                                                {/* <h6 className='mt-3'>{moment({}).seconds(flightinfo.data[0].Results.Segments[idx][0].Duration).format("HH:mm")}</h6> */}
                                                            </div>
                                                        </div>
