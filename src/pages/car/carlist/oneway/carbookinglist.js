                        ...
                        {(carlist.data[0].TaxiData).map((data, index) => (
                            ...
                            <div key={index} className="carbookinglist mb-3">
                                <Card >
                                    ...
                                    <Card.Body>
                                        <Row className="cab_content mt-4">
                                            <Col xs={9}>
                                                <ul>
                                                    <li key={index}><FontAwesomeIcon icon={faCircleCheck} className="text-primary" /><p>{data?.Fare?.TotalKmCharged} kms for {numberFormat(data?.Fare?.BaseFare, "INR")}. For extra kilometers {numberFormat(data.Fare.OutStationExtraKmRate, "INR")}/km</p></li>
                                                    ...
                                                    <li key={index}><FontAwesomeIcon icon={faCircleCheck} className="text-primary" /><p>Advance amount for this cab is {numberFormat(data.Fare.AdvanceAmount, "INR")} only</p></li>
                                                    <li key={index}><FontAwesomeIcon icon={faCircleCheck} className="text-primary" /><p>Outstation driver allowance is {numberFormat(data.Fare.OutStationDriverAllowance, "INR")} only</p></li>
                                                    <li key={index}><FontAwesomeIcon icon={faCircleCheck} className="text-primary" /><p>Waiting charges is {numberFormat(data.Fare.LocalExtraHrRate, "INR")}/Hrs</p></li>
