            ...
            <div className='w-[80%] mx-auto '>
                <div className='border my-5 absolute z-50'>
                    ...
                    <div className='bg-gradient-to-r from-cyan-400 to-blue-400 p-3'>
                        <p className='text-lg text-white text-center font-bold'>Booking Details</p>
                    </div>
                    <div className='flex flex-col lg:flex-row justify-start'>
                        <div className='w-full lg:w-1/2 p-2'>
                            {Object.entries(bookingData).slice(0, 1).map(([label, value]) => (
                        ...
                        <div className='w-full lg:w-1/2 p-2'>
                            {Object.entries(bookingData).slice(1, 2).map(([label, value]) => (
                                <div key={label} className="flex my-5 pl-5">
                                    <div className="font-bold lg:w-1/3">{label}:</div>
                                    <div className="">{value}</div>
                                </div>
                    ...
                    <div className='flex flex-col lg:flex-row justify-start p-2'>
                        <div className='w-full lg:w-1/2 p-2'>
                            {Object.entries(hotelData).map(([label, value], index) => (
                            ...
                            <thead>
                                <tr className='border-black border-b-2'>
                                    {Object.values(jsonData.tableHeading).map((heading, index) => (
                                        <th key={index} className='py-2 text-start'>{heading}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        <div className='w-full lg:w-1/2 p-2'>
                            ...
                            <ul>
                                {instructionData.instructions.map((instruction, index) => (
                                    <li key={index} className='py-2'>{instruction}</li>
                                ))}
                            </ul>
                        </div>
                    </div>
                ...
                <div className='relative'>
                    <div className='bg-cover lg:w-1/2 md:w-4/5 sm:w-full h-[400px] opacity-20' style={{ backgroundImage: `url(${room})` }}>

                    </div>
                    <div className='bg-cover w-[80%] h-[380px] opacity-20' style={{ backgroundImage: `url(${removebg})`, marginTop: '480px', marginLeft: '172px' }}>
                    </div>
                </div>
            </div>
