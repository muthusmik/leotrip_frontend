...
export function Avatar() {
  ...
  const MenuOptions = [
    ...
    },
    {
      ...
      icon: TripIcon,
      description:
        "Details of your upcoming, completed trips and manage your trips",
  ...
  return (
    <div ref={wrapperRef} className=" relative mx-2 ml-[5%]">
      {!auth ? (
        <div
          className="border-int-dark-blue border-2 rounded-[10px] p-2 flex "
          onClick={() => setIsOpenAuthModal(true)}
        >
          <img src={AvatarIcon} alt={""} />
          <span className="font-poppinsRegular text-lg text-int-dark-blue pe-3 mx-2 text-center text-wrap hidden lg:flex ">
            Login/Register
          </span>
        </div>
      ) : (
        <div
          className="p-2 flex cursor-pointer hover:bg-int-background rounded-[10px] "
          onClick={() => setIsDropdownOpen(!isDropdownOpen)}
        >
          <img
            src={AvatarIcon}
            alt={""}
            className="p-1 rounded-full  border-0 md:border-0 sm:border-0 border-int-dark-blue w-24 lg:w-10 md:w-10"
          />
          <span className="font-poppinsRegular text-sm md:text-lg text-black  mx-1 justify-content mt-2 text-center">
            Hi <b>Leo</b>
          </span>
        </div>
      )}
      {isDropdownOpen && (
        <div
          className={`absolute ${dropdownPosition} top-full left-0 right-auto shadow-top-dark mt-1 z-10 bg-white py-3 px-2 rounded-lg ${
            isDropdownOpen ? "animate-slide-in-top" : "animate-slide-out-top"
          }`}
          style={{
            left: dropdownPositionX,
            right: "auto",
            minWidth: "200px",
            maxWidth: "400px",
          }}
        >
          <div
            className="absolute w-5  h-5 border-[20px]   border-solid border-transparent border-b-white"
            style={{ top: "-28px", left: "80%", transform: "translateX(-50%)" }}
          ></div>
          <div className="relative">
            {MenuOptions.map((option, index) => (
              ...
              >
                ...
                <div>
                  <span className="font-poppinsRegular text-base text-gray-700">
                    {option.name}
                  </span>
      ...
      >
        <AuthLogin
          Back={handleLoginBack}
          UserAuthMedium={UserAuthMedium}
          verifyOTPProps={verifyOTPProps}
