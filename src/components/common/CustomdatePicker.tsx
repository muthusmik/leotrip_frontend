    ...
    const renderDaysByWeek = () => {
      ...
      for (let day = 1; day <= totalDays; day++) {
        ...
        currentWeek.push(
          ...
          >
            <div
              className={`mb-0 py-3 text-[14px] pointer-cursor
                                    hover:bg-[#F5EBEB] hover:text-black hover:rounded-[300px]
                                    ${
                                      isToday
                                        ? "rounded-[150px] bg-[#F5EBEB]"
                                        : ""
                                    }
                                    ${
                                      isSelected
                                        ? "text-white rounded-[300px] bg-[#BD3319]"
                                        : ""
                                    }
                                    ${
                                      isHighlighted
      ...
      return daysByWeek.map((week, index) => (
        <React.Fragment key={index}>
          <tr
            key={index}
            className={`h-14 ${
              index === daysByWeek.length - 1
                ? ""
                : "border-b-2 border-int-gray"
            } `}
          >
            {week}
          </tr>
    ...
    return (
      <div className="custom-datepicker cursor-pointer">
        <input
          ...
          className="h-10 rounded-[10px] font-poppinsRegular w-full text-xs text-sm md:text-md bg-transparent"
        />
        {isDatePickerVisible && (
          <div
            ref={wrapperRef}
            className="absolute mt-6 left-0 md:left-[50%] mdlg:left-auto md:pl-0 bg-white rounded-2xl shadow-lg  z-40 w-auto pe-1 "
          >
            <div
              className="flex mx-5 mt-1 bg-white items-center text-center justify-between"
            ...
            <table className="days-table mx-5" style={{ userSelect: "none" }}>
              ...
              <tbody className="user-select-none">{renderDaysByWeek()}</tbody>
            </table>
          </div>
        )}
      </div>
