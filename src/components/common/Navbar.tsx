...
function Navbar() {
  ...
  const navbarLinks = [
    {
      to: "/flights",
      label: "Flights",
      icon: isScrolled ? FlightShrinkIcon : FlightNavIcon,
    },
    {
      to: "/hotel",
      label: "Hotel",
      icon: isScrolled ? HotelShrinkIcon : HotelNavIcon,
    },
  ...
  const menuLinks = [
    {
      to: "/flights",
      label: "Flights",
      icon: isScrolled ? FlightShrinkIcon : FlightShrinkIcon,
    },
    {
      to: "/hotel",
      label: "Hotel",
      icon: isScrolled ? HotelShrinkIcon : HotelShrinkIcon,
    },
    {
      to: "/bus",
      label: "Bus",
      icon: isScrolled ? BusShrinkIcon : BusShrinkIcon,
    },
    {
      to: "/car",
      label: "Car",
      icon: isScrolled ? CarShrinkIcon : CarShrinkIcon,
    },
  ];
  const handlemenu = () => {
  ...
  return (
    <>
      <nav
        className={`navbar bg-white py-1 md:py-1 lg:py-2 shadow-bottom sticky top-0 ${
          isScrolled
            ? openMenu
              ? "flex flex-wrap h-[125px] animate-slide-in-top"
      ...
      >
        ...
        >
          <div className="flex ml-3 items-center space-x-6 md:space-x-16 md:ml-0 lg:ml-[5%] w-full">
            ...
            <div
              className={`hidden md:flex space-x-2 md:space-x-16 py-5 md:py-0 mt-1 w-[60%] `}
            >
              {navbarLinks.map((link) => (
                ...
                >
                  <div
                    className={`${isScrolled ? "h-[20px]" : "h-[30px"} mb-1 ${
                      isScrolled ? "scale-90" : ""
                    } transition-all duration-300`}
                  >
                    <img
