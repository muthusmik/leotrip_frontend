...
const RoomGuestCountComponent = ({
  roomGuestCount,
  setRoomGuestCount,
  showRoomGuestDropdown,
}: RoomGuestCountComponentProps) => {
  const [currentRoomCount, setCurrentRoomCount] = useState(
  ...
  useOutsideAlerter({
    ref: wrapperRef,
    callback: () => showRoomGuestDropdown(false),
  });
    ...
    for (let i = 0; i < currentChildCount; i++) {
      inputs.push(
        <div className="flex items-center justify-center w-[48%] mb-2">
          <p className="w-[30%] text-sm font-bold text-center">{`Child ${
            i + 1
          } age`}</p>
  ...
  const mainContainerStyle =
    "flex flex-row items-center justify-between px-8 py-2 w-full";
  const subContainerStyle =
    "flex flex-row items-center justify-center border-2 border-slate-400 rounded-lg w-[100px] h-[40px]";
  const imgContainerStyle =
    "w-[40%] h-full flex items-center justify-center self-center cursor-pointer";

  const imgStyle = "w-[70%] h-[72%]";
  const countContainerStyle =
    "border-l-2 border-r-2 h-full border-slate-400 items-center justify-center";
  const countTag =
    "w-[30px] h-full text-center flex items-center justify-center font-poppinsRegular";

  return (
    <div
      ref={wrapperRef}
      className="absolute  md:top-[12rem] sm:top-[27rem] top-[24rem] right-0    bg-white border-4 rounded-[10px] w-[100%] sm:w-[65%] md:[w-65%]  z-0"
    >
      <p className="text-center font-bold text-2xl font-poppinsRegular my-4">
        Rooms and Guests
      </p>
      <hr />
      <div className={mainContainerStyle}>
        <h2 className="text-center font-poppinsRegular text-lg">
          Rooms<span className="text-sm"> (Max 8)</span>
        </h2>
        <div className={subContainerStyle}>
          <div
            className={imgContainerStyle}
            onClick={() => handleRoomCount("DEC")}
          >
      ...
      <div className={mainContainerStyle}>
        <h2 className="text-center font-poppinsRegular text-lg">
          Children<span className="text-sm"> (0-17 yrs)</span>
        </h2>
        <div className={subContainerStyle}>
          <div
            className={imgContainerStyle}
      ...
      <div className="w-full px-2">
        <h6 className="text-center font-poppinsRegular text-sm mb-2">
        ...
        {currentChildCount > 0 && !isChildAgeValid() && (
          <p className="text-center text-red-700 font-poppinsRegular mt-2">
            Please provide ages for all children.
          </p>
        )}
      </div>
      <hr />
